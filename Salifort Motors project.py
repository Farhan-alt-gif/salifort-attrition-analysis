...
# ## **Pace: Plan**
#
# Consider the questions in my PACE Strategy Document to reflect on the Plan stage.
#
# In this stage, i consider the following:
...
# - What is my initial observations when i explore the data?
...
# 💭
# ### Reflect on these questions as i complete the analyze stage.
#
...
sns.scatterplot(data=df1, x='average_monthly_hours',
                y='satisfaction_level', hue='left', alpha=0.4)
...
sns.boxplot(data=df1, x='satisfaction_level',
            y='tenure', hue='left', orient="h", ax=ax[0])
ax[0].invert_yaxis()
...
#
# Next, i could explore whether there's a correlation between working long hours and receiving high evaluation scores. I could create a scatterplot of `average_monthly_hours` versus `last_evaluation`.
...
plt.figure(figsize=(16, 9))
sns.scatterplot(data=df1, x='average_monthly_hours',
                y='last_evaluation', hue='left', alpha=0.4)
plt.axvline(x=166.67, color='#ff6361', label='166.67 hrs./mo.', ls='--')
plt.legend(labels=['166.67 hrs./mo.', 'left', 'stayed'])
plt.title('Monthly hours by last evaluation score', fontsize='14')
...
# 💭
# ### Reflect on these questions as i complete the constructing stage.
#
...
# Since the variable you want to predict (whether an employee leaves the company) is categorical, i could either build a Logistic Regression model, or a Tree-based Machine Learning model.
#
# So i could proceed with one of the two following approaches. Or, if i'd like, i could implement both and determine how they compare.
...
# Create a heatmap to visualize how correlated variables are. Consider which variables i am interested in examining correlations between.
...
df_logreg = df_enc[(df_enc['tenure'] >= lower_limit) &
...
# Select the features i want to use in my model. Consider which variables will help me predict the outcome variable, `left`.
...
get_ipython().run_cell_magic(
    'time', '', 'rf1.fit(X_train, y_train) # --> Wall time: ~10min')


# Specify path to where i want to save your model.

# In[48]:


# Define a path to the folder where i want to save the model
...
def write_pickle(path, model_object, save_as: str):
    '''
    In: 
        path:         path of folder where i want to save the pickle
        model_object: a model i want to pickle
        save_as:      filename for how i want to save the model
...
def read_pickle(path, saved_model_name: str):
    '''
    In: 
        path:             path to folder where i want to read from
        saved_model_name: filename of pickled model i want to read in
...
def get_scores(model_name: str, model, X_test_data, y_test_data):
    ...
    In: 
        model_name (string):  How i want my model to be named in the output table
...
# I might be skeptical of the high evaluation scores. There is a chance that there is some data leakage occurring. Data leakage is when i use data to train my model that should not be used during training, either because it appears in the test data or because it's not data that i'd expect to have when the model is actually deployed. Training a model with leaked data can give an unrealistic score that is not replicated in production.
#
...
#
# To make the `overworked` column binary, i could reassign the column using a boolean mask.
...
get_ipython().run_cell_magic(
    'time', '', 'rf2.fit(X_train, y_train) # --> Wall time: 7min 5s')
...
#
# I can also get feature importance from decision trees (see the [DecisionTreeClassifier scikit-learn documentation](https://scikit-learn.org/stable/modules/generated/sklearn.tree.DecisionTreeClassifier.html#sklearn.tree.DecisionTreeClassifier.feature_importances_) for details).
...
# I can then create a barplot to visualize the decision tree feature importances.
...
#
# - What key insights emerged from my model(s)?
